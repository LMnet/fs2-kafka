        ...
        adminClientResource[IO](adminClientSettings(config))
          .use { adminClient =>
            for {
              ...
              _ <- IO(assert(consumerGroupOffsetsMap.map { case (_, offset) =>
                offset.offset()
              }.forall(_ > 0)))
              _ <- IO {
                adminClient
              ...
              _ <- adminClient.listConsumerGroupOffsets(consumerGroupId).partitionsToOffsetAndMetadata.map { res =>
              ...
              _ <- adminClient.listConsumerGroupOffsets(consumerGroupId).partitionsToOffsetAndMetadata.map { res =>
                val expected = consumerGroupOffsetsMap.removed(partition0)
                assert(res == expected)
              }
            } yield ()
          }
